        ...
        for i, day in enumerate(self.days):
            ...
            for ev in self.events_on_day(day):
                ...
                if tmp_size is not None:
                    ...
                    line_height = font_size * 1.2
        canvas.setFontSize(header_size)
        canvas.drawCentredString(300, 745, "UGANDA MARTYRS UNIVERSITY")
        canvas.drawCentredString(300, 730, subgroup)
        canvas.drawCentredString(300, 50,
                                 f'Generated by Anthony on {str(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))}')
...
def main(
    ...
    xlist = [x + x_offset for x in [0, 200, 300]]
